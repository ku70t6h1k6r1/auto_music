...
if __name__ == '__main__':
    #load lead sheet
    leadSheet = ls.composition_demo20180505()
    ...
    mel_articulation = np.stack([articuration *110], axis = -1)
    ...
    vc_articuration = np.stack([articuration*70 ,articuration*70], axis = -1)
    ...
    sp_melody_wav =  mltPrcss.ChildProcessWave(leadSheet.wav_dir, melody_dur, pointer_Harm , currentBeat, melody_wav, mel_articulation, timeSeriesObj, playFlg_mel, goFlg)
    sp_cMelody =  mltPrcss.ChildProcess(device, 0, 5, pointer_Harm, currentBeat, cMelody, cMel_articuration, timeSeriesObj, playFlg_mel, goFlg)
    sp_ba =  mltPrcss.ChildProcess(device, 1, 5, pointer_Harm, currentBeat, ba, ba_articulation, timeSeriesObj, playFlg_ba, goFlg)
    sp_vc =  mltPrcss.ChildProcess(device, 2, 5, pointer_Harm, currentBeat, vc, vc_articuration, timeSeriesObj, playFlg_vc, goFlg)
    sp_dr = mltPrcss.ChildProcess(device, 9, 0, pointer_Perc, currentBeat, dr, dr_articuration, timeSeriesObj, playFlg_dr, goFlg)
    sp_mDr = mltPrcss.ChildProcessMasudaDaw(port, 9, 0, pointer_Perc, currentBeat, mDr, dr_articuration, timeSeriesObj, playFlg_dr, goFlg)
    ...
    try:
        ...
        p8.start()
        """
        print("ALL SUB PROSESS HAS CALLED")
        p_seq.join()
...
if __name__ == 'not move':
    ...
    vc = np.stack([v1, v2 ], axis = -1)
    vc_articuration = np.stack([articuration*50 ,articuration*80], axis = -1)
    ...
    cMelody = np.stack([counterMelody], axis = -1)
    cMel_articuration = np.stack([articuration*80], axis = -1)
