...
void setup() {      
  ...
  myBus2 = new MidiBus(this, -1, "Yamaha UX16-1");
...
void draw() {
    ...
    Current_Note_2ch = Convert_midi(secondch_array,i,2,1,Current_Note_2ch,100);
    Current_Note_10_1ch = Convert_midi_dr(Dr_ba_ch_array,i,36,Current_Note_10_1ch);
    Current_Note_10_2ch = Convert_midi_dr(Dr_rcy_ch_array,i,46,Current_Note_10_2ch);
...
int Convert_midi_dr(int note_array[],int i, int inst, int Curent_Note){
  ...
  byte inst_data[] = new byte[2];
    ...
    //myBus.sendMessage(inst_data);
    Curent_Note = inst;
    myBus2.sendNoteOn(0x9, inst, 100); // Send a Midi noteOn
  }else{
  }
  return inst;
}